                                      ...
                                      const Ref<const Vector_dof> &_phi_1, const Ref<const Vector_dof> &_dphi_1, const Ref<const Vector_leg> &_c_1)
{
    ...
    result_sum_delta_epsilon.setZero();
    for (int j = 0; j < NUM_OF_LEG; j++)
        result_delta_epsilon.push_back(Eigen::Vector3d::Zero());
                                                ...
                                                Vector_rho &result_linearized_rho, bool update_jacobian)
{
    ...
    Matrix<double, 3, NUM_OF_LEG> lo_veocities;
    lo_veocities.setZero();
    for (int j = 0; j < NUM_OF_LEG; j++)
    {
    ...
    Vector3d average_delta_epsilon;
    average_delta_epsilon.setZero();
    Vector3d average_count;
    average_count.setZero();
    Vector12d weight_list;
    weight_list.setZero();
    ...
    noise_diag.diagonal() << (ACC_N * ACC_N), (ACC_N * ACC_N), (ACC_N_Z * ACC_N_Z),
    ...
    if (update_jacobian)
    {
        ...
        for (int j = 0; j < NUM_OF_LEG; j++)
        {
            ...
            F.block<3, 3>(ILO_EPS1 + 3 * j, ILO_BG) = 0.5 * _dt * _dt * result_delta_q.toRotationMatrix() * Utility::skewSymmetric(vip1[j]) - 0.5 * _dt * (delta_q.toRotationMatrix() * Utility::skewSymmetric(p_br + R_br * fi[j]) + result_delta_q.toRotationMatrix() * Utility::skewSymmetric(p_br + R_br * fip1[j]));
        ...
        for (int j = 0; j < NUM_OF_LEG; j++)
        {
            V.block<3, 3>(ILO_EPS1 + 3 * j, ILNO_Gi) = -0.25 * _dt * _dt * result_delta_q.toRotationMatrix() * Utility::skewSymmetric(vip1[j]) + 0.5 * _dt * delta_q.toRotationMatrix() * Utility::skewSymmetric(p_br + R_br * fi[j]);
            V.block<3, 3>(ILO_EPS1 + 3 * j, ILNO_Gi1) = -0.25 * _dt * _dt * result_delta_q.toRotationMatrix() * Utility::skewSymmetric(vip1[j]) + 0.5 * _dt * result_delta_q.toRotationMatrix() * Utility::skewSymmetric(p_br + R_br * fip1[j]);
